...
final class SailbuddyMapStyleController extends ControllerBase {
  ...
  public function build(string $chart = 'seamap'): JsonResponse {
    $cid = 'sailbuddy_map:style:' . $chart . ':v2';
    ...
    // where the bundled MapLibre version (< 5.20) would fail to parse or
    // render:
    ...
    //  - The "basics" sprite (tiles.versatiles.org) currently 404s upstream;
    //    MapLibre treats multi-sprite loading as all-or-nothing, so a failing
    //    sprite aborts the whole batch and every icon layer (buoys, beacons,
    //    ...) loses its images. Drop it so freenauticalchart loads alone.
    if (isset($style['sprite']) && is_array($style['sprite'])) {
      $style['sprite'] = array_values(array_filter($style['sprite'], function ($entry) {
        if (!is_array($entry)) {
          return TRUE;
        }
        $url = $entry['url'] ?? '';
        return strpos($url, 'versatiles') === FALSE;
      }));
    }
