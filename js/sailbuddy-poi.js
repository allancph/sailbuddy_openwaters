(function (Drupal, drupalSettings, once) {
  ...
  function addToLayerControl(map, layer, label) {
    var ctrl = null;
    (map._controls || []).forEach(function (c) {
      if (!ctrl && c instanceof L.Control.Layers) {
        ctrl = c;
      }
    });
    if (ctrl) {
      ctrl.addOverlay(layer, label);
    }
    else {
      var fallback = L.control.layers(null, {});
      fallback.addOverlay(layer, label);
      fallback.addTo(map);
    }
  }
